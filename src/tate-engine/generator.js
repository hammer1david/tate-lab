...
export function resolveVo2Intensity(
  ...
  const targetPaceSecondsPerKm = Math.max(
    1,
    baselinePaceSecondsPerKm +
      stateAdjustmentSecondsPerKm +
      responseAdjustmentSecondsPerKm
  ...
  if (latestResponse) {
    if (responseAdjustmentSecondsPerKm > 0) {
      reason =
        `VO2 response ${latestResponse.feedback}: ease pace by ${responseAdjustmentSecondsPerKm}s/km before adding work${repeatedTooMuch ? '; repeated Too much also permits dose reduction' : ''}.`;
...
function vo2Candidates(athlete) {
  ...
  for (const form of vo2Model.forms) {
    ...
    const repsList = vo2CandidateRepRange(
      form,
      repSeconds,
      doseRange
    );
...
function evaluateVo2History(
  ...
  if (sessionResponse.intensityAdaptation) {
    rotation = {
      ...
      fit: Math.max(rotation.fit, 0.9),
      reason:
        `${rotation.reason} Same-form rotation penalty relaxed for VO2 pace adaptation.`,
    };
    ...
    structuralRotation = {
      ...
      reason:
        `${structuralRotation.reason} Structural rotation is temporarily neutralized for VO2 pace-first adaptation.`,
    };
  ...
  const historySimilarityExposure = clamp(
    comparisons.reduce(
      (sum, item) =>
