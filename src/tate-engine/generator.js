...
export function resolveThresholdIntensity(athlete) {
  ...
  if (latestResponse) {
    if (adjustmentSecondsPerKm > 0) {
      reason =
        `Session response ${latestResponse.feedback}: ease threshold pace by ${adjustmentSecondsPerKm}s/km before cutting threshold time${repeatedTooMuch ? '; repeated Too much also allows dose reduction' : ''}.`;
    } else if (adjustmentSecondsPerKm < 0) {
      reason =
        `Session response ${latestResponse.feedback}: increase threshold pace by ${Math.abs(adjustmentSecondsPerKm)}s/km before adding threshold time.`;
    } else {
      reason =
        `Session response ${latestResponse.feedback}: keep threshold pace unchanged.`;
    }
  }
