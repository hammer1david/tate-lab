...
const LAB_VERSION = '0.3.2-phase-placeholders';
...
function renderPlan(plan) {
  ...
  $('plan-status').textContent = missing
    ? `${phaseLabel} · ${plan.slotCount} slots · ${missing} database gap${
        missing === 1 ? '' : 's'
      }`
    : `${phaseLabel} · ${plan.slotCount} slots · all assigned from Supabase`;
}
