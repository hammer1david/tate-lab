...
function renderDoseAndRotation(candidate) {
  const intensity =
    Number.isFinite(candidate.thresholdPaceSecondsPerKm)
      ? `
        <div class="meta-line">
          Threshold pace:
          <strong>${paceToClock(candidate.thresholdPaceSecondsPerKm)}/km</strong>
          ${
            Number.isFinite(candidate.thresholdPaceAdjustmentSecondsPerKm) &&
            candidate.thresholdPaceAdjustmentSecondsPerKm !== 0
              ? ` · response adjustment ${candidate.thresholdPaceAdjustmentSecondsPerKm > 0 ? '+' : ''}${candidate.thresholdPaceAdjustmentSecondsPerKm}s/km`
              : ''
          }
        </div>
        ${
          candidate.thresholdIntensityReason
            ? `<div class="meta-line">Intensity: ${candidate.thresholdIntensityReason}</div>`
            : ''
        }
      `
      : '';
  ...
  return intensity + dose + rotation + structure + response;
}
