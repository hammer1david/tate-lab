...
function renderDoseAndRotation(candidate) {
  ...
  const paceSecondsPerKm = isVo2
    ? candidate.vo2PaceSecondsPerKm
    : candidate.thresholdPaceSecondsPerKm;

  const paceAdjustment = isVo2
    ? candidate.vo2PaceAdjustmentSecondsPerKm
    : candidate.thresholdPaceAdjustmentSecondsPerKm;

  const intensityReason = isVo2
    ? candidate.vo2IntensityReason
  ...
  const intensity =
    Number.isFinite(paceSecondsPerKm)
      ? `
        <div class="meta-line">
          ${isVo2 ? 'VO₂ pace' : 'Threshold pace'}:
          <strong>${paceToClock(paceSecondsPerKm)}/km</strong>
          ${
            Number.isFinite(paceAdjustment) &&
            paceAdjustment !== 0
              ? ` · response adjustment ${paceAdjustment > 0 ? '+' : ''}${paceAdjustment}s/km`
              : ''
          }
        </div>
        ${
          intensityReason
            ? `<div class="meta-line">Intensity: ${intensityReason}</div>`
            : ''
        }
      `
      : '';
  ...
  const doseMinutes =
    Number.isFinite(candidate.qualityDoseMinutes)
      ? candidate.qualityDoseMinutes
      : candidate.thresholdDoseMinutes;

  const dose =
    Number.isFinite(doseMinutes)
      ? `
        <div class="meta-line">
          ${isVo2 ? 'VO₂ dose' : 'Threshold dose'}:
          <strong>${doseMinutes.toFixed(1)} min</strong>
          ${
            candidate.doseRange
  ...
  const repDuration =
    isVo2 &&
    Number.isFinite(candidate.vo2RepDurationSeconds)
      ? `
        <div class="meta-line">
          VO₂ rep duration:
          <strong>${paceToClock(candidate.vo2RepDurationSeconds)}</strong>
        </div>
      `
      : '';
  ...
  const reassessment =
    isVo2 &&
    candidate.performanceReassessmentNeeded
      ? `
        <div class="meta-line">
          <strong>Performance reassessment needed:</strong>
          VO₂ pace has reached the current domain ceiling.
        </div>
      `
      : '';
  ...
  return intensity + dose + repDuration + reassessment + rotation + structure + response;
}
